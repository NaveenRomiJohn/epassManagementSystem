<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
	pageEncoding="ISO-8859-1"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="ISO-8859-1">
<title>Index</title>
<style>
* {
	text-align: center;
}
</style>
</head>
<body>
	<h1>E-Pass Management System</h1>
	<h2>Covid-19 ePass</h2>
	<br>
	<hr/>
	<h4>KINDLY READ INSTRUCTIONS BELOW</h4>
	<hr/>
	<div class="tdata">
		<label>1.The pass holders are requested to carry the Driving
			License compulsorily </label>
	</div>
	<div class="tdata">
		<label>2. Strict actions shall be taken against those found
			misusing the e-pass </label>
	</div>
	<div class="tdata">
		<label>3. The state authority is liable to cancel the pass
			anytime </label>
	</div>
	<div class="tdata">
		<label>4. People on Government&#x2f;Medical&#x2f;Press Media
			should carry their respective ID Card. </label>
	</div>
	<div class="tdata">
		<label>5. The vehicle e-Pass is only for
			intrastate&#x2f;interstate travel </label>
	</div>
	<div class="tdata">
		<label>6. This Vehicle e-Pass is non-transferable </label>
	</div>
	<br>
	<a href="userloginform">User LogIn</a>
	<br>
	<br>
	<a href="adduserform">New User Register</a>
	<br>
	<br>
	<a href="adminloginform">Admin LogIn</a>
</body>
</html>